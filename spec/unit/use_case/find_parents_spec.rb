describe UseCase::FindParents do
  ...
  describe "#execute" do
    ...
    context "when we pass some uprns" do
    ...
    context "when we pass no uprns" do
      let(:result) do
        use_case.execute(uprns: [])
      end

      it "does not call the gateway if there are no uprns and returns []" do
        expect(result).to eq([])
        expect(addresses_gateway).not_to have_received(:search_by_uprns)
      end
    end
  end
end
